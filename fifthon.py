...
async def _(event):
    ...
    await event.edit(f'''
● ھـَْـُلاٌٍے وًَ غ̷̷َـلاَّ ۨشغال اࠗلسܱܰورس ●
● سـورس بلاك● @q_3_i ● @Source_BLACK1 ●
● البنك : `{ms}`●
● التاريخ: `{m9zpi}`●
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('● ● ● @q_3_i ● ● ●')


@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.اعادة تشغيل"))
async def update(event):
    await event.edit(" ثـواني ويشتغل ؟ لمن مايشتغل طب بقناتي  @Source_BLACK1 ")
